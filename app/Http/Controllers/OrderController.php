...
class OrderController extends Controller
{
    ...
    public function toAntrian(string $id){
        $order = Order::find($id);
        $platforms = DB::table('platforms')->where('platform_name','LIKE','%'.'Iklan'.'%')->get();

        return view ('page.antrian-workshop.create', compact('order', 'platforms'));
    }
    ...
    public function getJobsByCategory(Request $request, string $category_id){
        ...
        $jobs = Job::where('job_type', $category_id)->where('job_name', 'like' , '%'.$request->q.'%')
